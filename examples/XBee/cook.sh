...
EXAMPLES_DIR="$BASE_DIR/examples/XBee"
LIBRARY_DIR="$BASE_DIR/libraries/arduPiXBee"
...
if [ -e $file ]; then
  if [ "$1" == "-clean" ]; then
    echo "arduPiXBee.o -> purged"
    rm ./arduPiLoRa.o
  else
	echo "arduPiXBee already compiled"
  fi
else
  if [ "$1" != "-clean" ]; then
	echo "Compiling arduPiXBee..."
    g++ -c arduPiXBee.cpp \
      -I"$ARDUPIAPI_DIR" \
      -I"$ARDUPI_DIR" \
      -o arduPiXBee.o
