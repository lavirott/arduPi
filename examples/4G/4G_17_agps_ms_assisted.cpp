...
void setup()
{
  ...
  if (error == 0)
  {
    ...
    gps_status = _4G.gpsStart(arduPi4G::GPS_MS_ASSISTED);

    // check answer
    if (gps_status == 0)
    {
      printf("2. GPS started in MS-ASSISTED. Time(secs) = %d\n", (millis()-previous)/1000);
    }
    else
    {
...
void loop()
{
  ...
  else
  {
    ...
    // update variable
    gps_autonomous_needed = true;

    // check answer
    if (gps_status == 0)
    {
      printf("GPS started in MS-ASSISTED. Time(ms) = %d\n", millis() - previous);
    }
    else
    {
      printf("Error calling the 'gpsStart' function. Code: %d\n", gps_status);
