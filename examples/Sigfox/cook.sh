...
EXAMPLES_DIR="$BASE_DIR/examples/Sigfox"
LIBRARY_DIR="$BASE_DIR/libraries/arduPiSigfox"
...
if [ -e $file ]; then
  ...
  if [ "$1" != "-clean" ]; then
	echo "Compiling arduPiSigfox..."
    g++ -c arduPiSigfox.cpp \
      -I"$ARDUPIAPI_DIR" \
      -I"$ARDUPI_DIR" \
      -o arduPiSigfox.o 
  fi
