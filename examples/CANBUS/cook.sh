...
EXAMPLES_DIR="$BASE_DIR/examples/CANBUS"
LIBRARY_DIR="$BASE_DIR/libraries/arduPiCANBUS"
...
if [ -e $file ]; then
  if [ "$1" == "-clean" ]; then
    echo "arduPiCAN.o -> purged"
    rm ./arduPiCAN.o
  else
	echo "arduPiCAN already compiled"
  fi
else 
  if [ "$1" != "-clean" ]; then
	echo "Compiling arduPiCAN..."
    g++ -c arduPiCAN.cpp -o arduPiCAN.o 
  fi
fi 
